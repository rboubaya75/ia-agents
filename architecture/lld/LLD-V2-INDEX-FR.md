...
Ce LLD relève trois écarts de corpus qu'il tranche sans les corriger lui-même : `capability-allocation-matrix.md` Domaine 1 décrit encore la propagation de claims par en-têtes annulée par `V2-ADR-020` et lui délègue un format sans objet ; `custom:tenantId` en claim obligatoire (`V2-LLD-001 §7.1.3`) contredit la résolution serveur de `V2-ADR-006` et force le jeton d'identité là où l'ADR nomme le jeton d'accès ; la formule de namespace Memory (`V2-LLD-003 §5.3`) reposerait sur un condensat tronqué à 48 bits, adapté aux journaux mais non à une frontière d'isolation.
