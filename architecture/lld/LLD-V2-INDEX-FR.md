...
| V2-LLD-004 | AgentCore Gateway MCP et tools | catalogue et classe de tool, schémas stricts, IAM, deux tools par action mutante, exécution par référence, idempotence par commande, retry et disjoncteur | V2-ADR-002, V2-ADR-005, V2-ADR-006, V2-ADR-008, V2-ADR-014 | v0.1 (Draft) |
...
Ce LLD relève quatre écarts de corpus qu'il ne corrige pas lui-même. Le premier est bloquant : le **magasin de commandes** exigé par `V2-ADR-014` n'a pas de propriétaire — l'ADR le délègue à `V2-LLD-006`, dont le §1.2 le renvoie à `V2-LLD-004` et `V2-LLD-005`. Une boucle de délégation ne produit pas un propriétaire, et l'ADR étant l'autorité, le magasin revient à `V2-LLD-006`. Le deuxième : `V2-LLD-006 §5.1` garde les mutations `Trips` dans la portée du ledger d'idempotence, dont `V2-ADR-014` les a retirées — deux enregistrements pour la même mutation, dont un seul est lu. Le troisième : l'endpoint de confirmation n'est pas nommé au tableau des routes de `V2-LLD-001 §7.0`, alors que `responseTransferMode` est un réglage par méthode. Le quatrième : deux corrections demandées par `V2-ADR-014` restent non appliquées dans `V2-ADR-006` (la commande confirmée comme fondement d'autorisation) et `V2-ADR-011` (la fenêtre non annulable est l'exécution, pas l'appel de tool entier).
