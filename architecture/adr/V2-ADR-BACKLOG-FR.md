...
| V2-ADR-017 | Classification documentaire et conservation — **rédigé, `Draft` en attente de revue** (`V2-ADR-017-classification-documentaire-conservation.md`). Fixe le domaine de valeurs de l'attribut `classification` introduit par `V2-ADR-003`, sa composition avec l'ACL et la source de vérité du filtre ; tranche que la conservation reste indépendante de la classification | V2-ADR-003, V2-ADR-006, V2-ADR-014, V2-ADR-015, V2-ADR-019 |
| V2-ADR-018 | Stratégie de tests RAG et seuils de qualité — **rédigé, `Draft` en attente de revue** (`V2-ADR-018-tests-rag-seuils-qualite.md`). Écarte les seuils absolus au profit d'une non-régression contre une baseline versionnée ; sépare le dataset de retrieval (déterministe, bloquant) du dataset de génération (non déterministe, publié) ; impose un corpus de fixtures figé, indépendant du corpus de production | V2-ADR-003, V2-ADR-010, V2-ADR-012, V2-ADR-013, V2-ADR-017, V2-ADR-019 |
