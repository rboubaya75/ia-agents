...
def create_gateway_lambda(lambda_function_code_path, region=region) -> dict[str, int]:
    ...
    print("Reading code from Python file")
    import zipfile
    import io
    
    # Create zip in memory from Python file
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        zip_file.write(lambda_function_code_path, 'lambda_function.py')
    
    lambda_function_code = zip_buffer.getvalue()
    ...
    if role_arn != "":
        ...
        try:
            ...
            # Wait for Lambda to become Active before returning
            print(f"Waiting for Lambda function '{lambda_function_name}' to become Active...")
            max_wait = 120
            elapsed = 0
            while elapsed < max_wait:
                func_resp = lambda_client.get_function(FunctionName=lambda_function_name)
                state = func_resp['Configuration'].get('State', 'Unknown')
                print(f"Lambda state: {state} (waited {elapsed}s)")
                if state == 'Active':
                    print(f"✅ Lambda function is now Active")
                    break
                elif state == 'Failed':
                    raise Exception(f"Lambda function creation failed: {func_resp['Configuration'].get('StateReason', 'unknown')}")
                time.sleep(5)
                elapsed += 5
            else:
                raise Exception(f"Lambda function did not become Active within {max_wait} seconds")
