...
Runtime injects authenticated identity, request correlation, deadlines and
idempotency keys. DynamoDB access remains partition-scoped and mutation retries
return the original outcome without duplicating side effects.
"""
...
MIN_DEADLINE_REMAINING_MS = int(os.getenv("TRIPS_MIN_DEADLINE_REMAINING_MS", "100"))
...
if MIN_DEADLINE_REMAINING_MS < 0 or MIN_DEADLINE_REMAINING_MS > 5000:
    raise RuntimeError("TRIPS_MIN_DEADLINE_REMAINING_MS must be between 0 and 5000.")
...
CONTEXT_FIELDS = {"userId", "requestId", "deadlineEpochMs"}
CREATE_FIELDS = CONTEXT_FIELDS | {
    "operationId",
    "tripName",
    "startDate",
    "endDate",
...
GET_TRIPS_FIELDS = CONTEXT_FIELDS | {"limit", "nextToken"}
INTERNAL_TRIP_FIELDS = {
    "userId",
    "creationOperationId",
...
def _public_trip(item: Dict[str, Any]) -> Dict[str, Any]:
    return _json_safe(
        {key: value for key, value in item.items() if key not in INTERNAL_TRIP_FIELDS}
    )
...
def _string(
    event: Dict[str, Any], field: str, max_length: int, *, required: bool = False
) -> str | None:
...
def _bounded_integer(
    event: Dict[str, Any], field: str, default: int, maximum: int
) -> int:
...
def _request_id(event: Dict[str, Any]) -> str:
    value = _string(event, "requestId", 128, required=True)
    assert value is not None
    if not REQUEST_ID_PATTERN.fullmatch(value):
        raise ValidationError("requestId has an invalid format.")
    return value


def _operation_id(event: Dict[str, Any]) -> str:
    value = _string(event, "operationId", 36, required=True)
    assert value is not None
    if not OPERATION_ID_PATTERN.fullmatch(value):
        raise ValidationError("operationId must be a UUID.")
    return value.lower()


def _deadline_epoch_ms(event: Dict[str, Any]) -> int:
    value = event.get("deadlineEpochMs")
    if isinstance(value, bool) or not isinstance(value, int):
        raise ValidationError("deadlineEpochMs must be an integer.")
    if value - int(time.time() * 1000) <= MIN_DEADLINE_REMAINING_MS:
        raise ValidationError("Request deadline exceeded.")
    return value
...
def _iso_date(
    event: Dict[str, Any], field: str, *, required: bool = False
) -> str | None:
...
def _deterministic_trip_id(user_id: str, operation_id: str) -> str:
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"trip-create:{user_id}:{operation_id}"))
...
def _encode_next_token(last_key: Dict[str, Any], user_id: str) -> str:
    ...
    payload = json.dumps({"v": 1, "tripId": trip_id}, separators=(",", ":")).encode(
        "utf-8"
    )
...
def _decode_next_token(token: Any, user_id: str) -> Dict[str, str] | None:
    ...
    if (
        ...
        or set(value) != {"v", "tripId"}
        or value.get("v") != 1
    ):
        raise ValidationError("nextToken is invalid.")
    return {
        "userId": user_id,
        "tripId": _trip_id_value(value.get("tripId"), "nextToken tripId"),
    }


def _validate_context(event: Dict[str, Any]) -> tuple[str, str]:
    request_id = _request_id(event)
    _deadline_epoch_ms(event)
    return _user_id(event), request_id
...
def create_trip(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    user_id, request_id = _validate_context(event)
    ...
    optional_fields = _validate_optional_fields(event)
    operation_payload = {
        "tripName": trip_name,
    ...
    trip_id = _deterministic_trip_id(user_id, operation_id)
    now = _utc_now()
    item: Dict[str, Any] = {
        "userId": user_id,
        "tripId": trip_id,
    ...
    try:
        ...
        if (
            ...
            raise
        existing = table.get_item(Key={"userId": user_id, "tripId": trip_id}).get(
            "Item"
        )
        if (
        ...
        json.dumps(
            {
                ...
                "operation_hash": _safe_hash(operation_id),
                "trip_hash": _safe_hash(trip_id),
                "replayed": replayed,
            }
        )
    )
...
def get_trips(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    user_id, _ = _validate_context(event)
...
def get_trip(event: Dict[str, Any]) -> Dict[str, Any]:
    _reject_unexpected(event, CONTEXT_FIELDS | {"tripId"})
    user_id, _ = _validate_context(event)
...
def update_trip(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _require(event, ["userId", "requestId", "deadlineEpochMs", "operationId", "tripId"])
    user_id, request_id = _validate_context(event)
    operation_id = _operation_id(event)
    trip_id = _trip_id(event)
    table = _get_table()
    ...
    operation_hash = _operation_hash({"tripId": trip_id, **updates})
    if existing.get("lastOperationId") == operation_id:
        if existing.get("lastOperationHash") != operation_hash:
            raise IdempotencyConflictError(
                "operationId was already used with a different update request."
            )
        return {
            "updated": True,
            "replayed": True,
    ...
    try:
        ...
        if (
            ...
            raise
        current = table.get_item(Key={"userId": user_id, "tripId": trip_id}).get("Item")
        if isinstance(current, dict) and current.get("lastOperationId") == operation_id:
            if current.get("lastOperationHash") != operation_hash:
                raise IdempotencyConflictError(
                    "operationId was already used with a different update request."
                ) from exc
            replayed = True
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    request_id = (
        ...
        and REQUEST_ID_PATTERN.fullmatch(raw_request_id)
        else "unknown"
    )
    logger.info(
        json.dumps(
    ...
    try:
        ...
        logger.exception(
            "trip_tool_error tool=%s request_id=%s", tool_name or "unknown", request_id
        )
        raise
