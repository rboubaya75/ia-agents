...
## 5. Stratégie CI/CD rationalisée
...
Cette pipeline est responsable des ressources Terraform : Cognito, DynamoDB, S3/CloudFront, API Gateway, ECR, IAM, AgentCore Gateway, Runtime, Memory, tools, observabilité et budgets.

### 5.2 Pipeline applicative consolidée
...
Le mode `full` doit évoluer vers le parcours applicatif complet corrigé :
...
## 6. Stratégie de déploiement frontend

Le frontend est déployé via la pipeline applicative consolidée.

### 6.1 Déploiement statique initial
...
### 6.2 Redéploiement complet

Après création d’API Gateway, AgentCore Gateway, Runtime, Memory et targets, utiliser :
...
Ce mode doit activer le parcours applicatif complet :

```text
Browser -> CloudFront -> React App -> API Gateway -> AgentCore Gateway -> AgentCore Runtime
```
...
## 8. Étapes de travail recommandées
...
## 9. Points P0 avant codage massif
